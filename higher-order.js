...
let evenNumbers = mixedNumbers.filter((num, index, array) => {
  if (num % 2 === 0) {
    return true;
  } else return false;
});
...
let totalPopulation = populations.reduce(
  (runningTotal, curElement, curIndex, wholeArray) => {
...
let orderTotals = orders.map((order, index, arr) => {
...
let bobsTotal = purchases.reduce((sum, purchase) => {
  console.log(purchase.owner, purchase.price, sum);
